...
- `grep -q '<item>'` — 에러 페이지나 빈 응답을 커밋하는 것을 막는다. 없으면 피드가 깨진 날
  routine이 빈 파일을 읽는다.
- **내부 빌드 제거 단계** — 전 항목이 `No user-facing changes.`인 `<item>`을 미러에서 뺀다.
  routine 프롬프트의 같은 규칙은 LLM 판단이라 100%가 아니어서, 결정론적 1차 방어를 여기 둔다.
  히스토리 35개 버전 전수 검사 결과 문구는 단일 변형이었고, 걸러진 건 4건(v1.30096.5, v1.37937.2, v1.37937.3, v1.46388.4)뿐이다.
  일부 섹션만 빈 릴리스는 건드리지 않는다. 문구가 바뀌면 필터가 fail-open으로 꺼지고 프롬프트 규칙이 받는다.
