...
```
Actions (30분마다)                     routine (매일 08·10·14·16시 KST)
...
- **전 섹션 `No user-facing changes.`인 릴리스는 건너뜀** — 내부 빌드라 채널에 나가도 읽는 사람이 얻는 게 없다.
  피드 15건 중 3건(20%)이 이 경우였고, 그때 나간 메시지는 요약·스레드 모두 "사용자 노출 변경 사항 없음"만 반복했다.
  한 섹션이라도 실제 항목이 있으면 보낸다 — 판단 기준은 항목 개수가 아니라 "전부 비었는가" 하나뿐이다.
...
- **정시(`:00`)를 피한 것도 의도다.** GitHub 스케줄은 정시에 큐가 몰려 지연·유실이 잦다.
  ...
  15분 간격(`7,22,37,52`)도 시도했으나 스케줄이 아예 등록되지 않아 30분으로 완화했다.
  워크플로 수정 후 스케줄 반영에는 시간이 걸린다 — 그 사이 미러가 멈춰도 routine의
  48시간 판정 창이 흡수하므로 릴리스를 놓치지는 않는다.
